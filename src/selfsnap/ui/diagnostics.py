...
def last_activity_summary(record: CaptureRecord | None) -> DiagnosticSummary:
    if record is None:
        return DiagnosticSummary(
            ...
            detail=(
                "Use Capture Now from Settings or the tray to validate storage, notifications, "
...
def storage_summary(config: AppConfig, paths: AppPaths) -> DiagnosticSummary:
    return DiagnosticSummary(
        ...
        detail=(
            f"Capture: {paths.resolve_capture_root(config)}\n"
            f"Archive: {paths.resolve_archive_root(config)}"
        ),
...
def retention_summary(config: AppConfig) -> DiagnosticSummary:
    ...
    if config.retention_mode == "keep_forever":
        detail = (
            "Automatic archival and purge are off. Files stay in the capture root until you move them."
        )
    else:
    ...
    return DiagnosticSummary(headline=headline, detail=detail)
