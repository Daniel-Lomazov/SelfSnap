...
This maintainer doc captures the current runtime, storage, scheduler, and UI architecture contract.
...
- Capture mode supports a composite image by default or one image per monitor in `per_monitor` mode.
...
- Settings keeps local editable state separate from polled disk-config state.
- Polling must not overwrite unsaved local schedule-enable edits; sync applies only when the local baseline is clean or after save updates that baseline.
