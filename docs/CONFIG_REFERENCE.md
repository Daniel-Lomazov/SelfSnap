...
| `retention_days` | archive-after days value | Used when retention mode requires it |
...
| `capture_mode` | capture strategy | `composite` or per-monitor mode |
...
- archive-after mode with `retention_days`: older captures move into archive
...
| Key | Common Values |
|---|---|
| `capture_mode` | `composite`, per-monitor mode |
...
| `interval_unit` | recurrence unit such as `minute`, `day`, or `week` |
